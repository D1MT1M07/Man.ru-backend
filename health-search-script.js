/**
 * Health Page Search Script
 * Поиск только по статьям категории "Здоровье"
 */

class HealthSearchManager {
    constructor() {
    ...
    performSearch(query) {
        ...
        // Ищем только в статьях категории "Здоровье"
        ...
        this.searchResultsContainer.innerHTML = foundArticles.map(article => {
            ...
            return `
                <div class="search-result-item" data-id="${itemId}" data-type="article" data-article-id="${article.id}" style="padding: 12px 15px; border-bottom: 1px solid #eee; cursor: pointer; transition: background 0.2s;">
                    <div style="display: flex; align-items: flex-start; gap: 10px;">
                        <div style="font-size: 20px; min-width: 24px;">❤️</div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.healthSearchManager = new HealthSearchManager();
});
